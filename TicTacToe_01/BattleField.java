...
public class BattleField {
	 ...
	 */
	public void makeField(String startCondition) {
		int k = 0;
		for (int i = 0; i < field.length; i++) {
			for (int j = 0; j < field[i].length; j++) {
				if (i == 0 || i == field.length - 1) {
					...
					field[i][j] = '|';
				} else if (j % 2 == 0 && j < field[i].length - 1) {
					if (startCondition.charAt(k) == '_') {
						field[i][j] = ' ';
					} else {
						field[i][j] = startCondition.charAt(k);
						if (startCondition.charAt(k) == 'X') {
							xNumber = getxNumber() + 1;
						}
						if (startCondition.charAt(k) == 'O') {
							oNumber = getoNumber() + 1;
						}
					}
					k++;
				} else {
