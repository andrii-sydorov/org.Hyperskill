...
public class Game {
	...
	public void start() {
		...
		gameInitiliazation();
		String startCondition = sc.nextLine();
		b.makeField(startCondition);
		b.printField();
		User currentUser = u[0];
		while (true) {
			currentUser.askUser();
			if (checkNotInstanceOfInteger(currentUser) || checkNotInRange() || checkIsOccupied() || checkLength(currentUser)) {
				continue;
			}
			break;
		}
